...
int main()
{
  ...
  try
  {
    ...
    assert_almost_equal(mesh.getVolume(), 1.5);  // ∫(x+1)dx = 3/2
    ...
    assert_almost_equal(mesh2.getVolume(), 1.5);  // ∫(2-x)dx = 3/2
    ...
    assert_almost_equal(mesh.getVolume(), 1.0);  // ∫(0-(-1))dx = 1
    mesh = mesher_cst.build(f_zero, 1, -1.0, 1.0, 1, false);
    assert_almost_equal(mesh.getVolume(), 1.0);  // ∫(1-0)dx = 1
    ...
    assert_almost_equal(mesh.getVolume(), 2.0);  // ∫(x0+x1+1) = 2

    // subGraph=False
    mesh = mesher3.build(f2d, 2, -1.0, 3.0, 1, false);
    fullprint << "  supergraph volume: " << mesh.getVolume() << std::endl;
    assert_almost_equal(mesh.getVolume(), 2.0);  // ∫(3-x0-x1) = 2
