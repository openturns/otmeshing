...
Mesh IntersectionMesher::build(const OT::Mesh & mesh1, const OT::Mesh & mesh2) const
{
  ...
  if (recompress_)
  {
    ...
    {
      const Indices nearest(tree.queryK(vertices[i], 2));
      const UnsignedInteger minIndex = std::min(nearest[0], nearest[1]);
      const UnsignedInteger candidateIndex = compressedIndices.find(minIndex);
      if (candidateIndex < compressedIndices.getSize())
      {
        // closest to point #i is already marked
        compressedVertexMap[i] = candidateIndex;
      }
      else
      {
        // mark point #i
        compressedVertexMap[i] = compressedIndices.getSize();
